...
export default function DiagnosticPatrimonial() {
  ...
  const hasFilledData = useMemo(() => {
    ...
    const hasChangedDefaults = revenus !== defaultRevenus || depenses !== defaultDepenses || epargne !== defaultEpargne || liquidites !== defaultLiquidites || situationFamiliale !== defaultSituationFamiliale || nombreEnfants !== defaultNombreEnfants;
    const hasAddedValues = assuranceVie > 0 || per > 0 || peaCto > 0 || residencePrincipale > 0 || immobilierLocatif > 0 || loyersPercus > 0 || creditsImmo > 0 || creditsRestants > 0 || donationsRealisees > 0 || perUtilise || lmnpUtilise || assuranceVieBeneficiaire;
  ...
  return <PublicPageLayout title="Diagnostic Patrimonial Gratuit | Éclat Toolkit" description="Analysez votre situation patrimoniale en 5 minutes. Remplissez le formulaire et découvrez votre score personnalisé.">
      <div className="space-y-8 pb-8">
        ...
        <motion.div className="sticky top-4 z-20 bg-white/95 backdrop-blur-sm rounded-xl p-4 shadow-sm border" initial={{
        opacity: 0,
        y: -10
      }} animate={{
        opacity: 1,
        y: 0
          ...
          <div className="relative h-2 bg-muted rounded-full overflow-hidden mb-4">
            <motion.div className="absolute inset-y-0 left-0 bg-gradient-to-r from-primary to-violet-500 rounded-full" initial={{
            width: 0
          }} animate={{
          ...
          <div className="flex items-center justify-between">
            ...
            return <div key={step.id} className="flex flex-col items-center flex-1">
                  <button onClick={() => handleStepClick(step.id)} disabled={!isClickable} className={`
                      w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-all
                      ${isCompleted ? "bg-primary text-white cursor-pointer hover:bg-primary/90" : isCurrent ? "bg-primary text-white ring-4 ring-primary/20" : "bg-muted text-muted-foreground cursor-not-allowed"}
                    `}>
        ...
        <AnimatePresence mode="wait">
          ...
          duration: 0.3
            ...
            {currentStep === 1 && <Card className="border shadow-sm">
                <CardContent className="pt-6">
                  <h2 className="text-lg font-semibold mb-4">1. Finances Personnelles</h2>
                  <FinancesPersonnellesCard revenus={revenus} setRevenus={setRevenus} depenses={depenses} setDepenses={setDepenses} epargneMensuelle={epargne} setEpargneMensuelle={setEpargne} creditsRestants={creditsRestants} setCreditsRestants={setCreditsRestants} />
                </CardContent>
              </Card>}
            
            {currentStep === 2 && <Card className="border shadow-sm">
                <CardContent className="pt-6">
                  <h2 className="text-lg font-semibold mb-4">2. Épargne & Investissements</h2>
                  <EpargneInvestissementsCard liquidites={liquidites} setLiquidites={setLiquidites} assuranceVie={assuranceVie} setAssuranceVie={setAssuranceVie} per={per} setPer={setPer} peaCto={peaCto} setPeaCto={setPeaCto} />
                </CardContent>
              </Card>}
            
            {currentStep === 3 && <Card className="border shadow-sm">
                <CardContent className="pt-6">
                  <h2 className="text-lg font-semibold mb-4">3. Immobilier</h2>
                  <ImmobilierCard residencePrincipale={residencePrincipale} setResidencePrincipale={setResidencePrincipale} immobilierLocatif={immobilierLocatif} setImmobilierLocatif={setImmobilierLocatif} loyersPercus={loyersPercus} setLoyersPercus={setLoyersPercus} creditsImmo={creditsImmo} setCreditsImmo={setCreditsImmo} />
                </CardContent>
              </Card>}
            
            {currentStep === 4 && <Card className="border shadow-sm">
                <CardContent className="pt-6">
                  <h2 className="text-lg font-semibold mb-4">4. Fiscalité</h2>
                  <FiscaliteCard revenusImposables={revenusImposables} setRevenusImposables={setRevenusImposables} tmi={tmi} setTmi={setTmi} perUtilise={perUtilise} setPerUtilise={setPerUtilise} lmnpUtilise={lmnpUtilise} setLmnpUtilise={setLmnpUtilise} />
                </CardContent>
              </Card>}
            
            {currentStep === 5 && <Card className="border shadow-sm">
                <CardContent className="pt-6">
                  <h2 className="text-lg font-semibold mb-4">5. Transmission & Protection</h2>
                  <TransmissionCard situationFamiliale={situationFamiliale} setSituationFamiliale={setSituationFamiliale} nombreEnfants={nombreEnfants} setNombreEnfants={setNombreEnfants} donationsRealisees={donationsRealisees} setDonationsRealisees={setDonationsRealisees} assuranceVieBeneficiaire={assuranceVieBeneficiaire} setAssuranceVieBeneficiaire={setAssuranceVieBeneficiaire} patrimoineTotal={patrimoineTotal} />
                </CardContent>
              </Card>}
        ...
        delay: 0.2
          ...
          {hasFilledData ? (/* Locked Results Preview with Visual Teasing - Shown when user has entered data */
        <Card className="border border-primary/20 bg-gradient-to-br from-white to-primary/5 overflow-hidden">
              <CardContent className="pt-6 space-y-6">
                ...
                scale: 0.98
                    ...
                    <Button onClick={handleViewResults} size="lg" className="gap-2 px-8 py-6 text-base shadow-xl bg-gradient-to-r from-primary to-violet-600 hover:from-primary/90 hover:to-violet-600/90">
            ...
            </Card>) : (/* Placeholder when no data has been entered yet */
